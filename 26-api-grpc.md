...
The relayer is a stateless gRPC service that:
...
3. Assigns a per-market sequence.
...
message SubmitIntentResponse {
  uint64 sequence       = 1;  // assigned per-market FIFO position
...
`SubmitIntent` returns after the relayer has committed the intent
on chain (i.e. the commit tx is submitted and accepted by the
RPC). Reveal/execute happens later; the response does **not** wait
for the fill.
